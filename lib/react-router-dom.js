  ...
  if (!ishandlerActive) {
    document.body.addEventListener("click", (e) => {
      if (e.target.matches("[data-link]")) {
        e.preventDefault();
        history.pushState(null, "", e.target.pathname || e.target.dataset.href);
        reRender(true);
      }
    });
...
const makeChildrenLink = (child, href) => {
  if (child.type) {
    child.props["data-link"] = true;
    child.props["data-href"] = href;
    child.children.forEach((subChild) => {
      makeChildrenLink(subChild, href);
    });
  }
